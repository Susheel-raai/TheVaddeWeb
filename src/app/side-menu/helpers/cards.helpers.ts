import {CardBrandEnum} from './../../../Models/CardValidationModel'

const digitMask = (numDigits: number) => Array(numDigits).fill(/\d/);

/*export const getValidationConfigFromCardNo = (rawValue: string): CardValidation =>cards.find(card => {
      const patterns = card.patterns.map(pattern => new RegExp(`^${pattern}`, 'g'));
      const matchResult = patterns.map(pattern => rawValue.match(pattern)).filter(result => result);
